...
static inline void obs_blend_alpha_neon(uint8_t *dst, const uint8_t *src, const uint8_t *alpha, size_t count)
{
	size_t i = 0;
	...
	for (; i + 16 <= count; i += 16) {
		...
		uint8x16x4_t alpha_expanded = {vdupq_laneq_u8(alpha_data, 0), vdupq_laneq_u8(alpha_data, 1),
					       vdupq_laneq_u8(alpha_data, 2), vdupq_laneq_u8(alpha_data, 3)};

		// Blend: dst = dst * (1 - alpha) + src * alpha
		// Calculate src * alpha
		uint16x8_t src_alpha_lo = vmull_u8(vget_low_u8(src_data), vget_low_u8(alpha_expanded.val[0]));
		uint16x8_t src_alpha_hi = vmull_u8(vget_high_u8(src_data), vget_high_u8(alpha_expanded.val[0]));

		// Calculate dst * (1 - alpha)
		uint8x16_t inv_alpha = vsubq_u8(vdupq_n_u8(255), alpha_expanded.val[0]);
		uint16x8_t dst_inv_alpha_lo = vmull_u8(vget_low_u8(dst_data), vget_low_u8(inv_alpha));
		uint16x8_t dst_inv_alpha_hi = vmull_u8(vget_high_u8(dst_data), vget_high_u8(inv_alpha));

		// Combine: (dst * (1-alpha) + src * alpha) / 255
		uint16x8_t blend_lo = vaddq_u16(dst_inv_alpha_lo, src_alpha_lo);
		uint16x8_t blend_hi = vaddq_u16(dst_inv_alpha_hi, src_alpha_hi);

		uint8x16_t result =
			vcombine_u8(vqmovn_u16(vshrq_n_u16(blend_lo, 8)), vqmovn_u16(vshrq_n_u16(blend_hi, 8)));
...
static inline void obs_blend_alpha_generic(uint8_t *dst, const uint8_t *src, const uint8_t *alpha, size_t count)
{

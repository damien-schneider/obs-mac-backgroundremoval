...
static CVPixelBufferRef create_iosurface_pixel_buffer(uint32_t width, uint32_t height, IOSurfaceRef *out_surface)
{
	...
	CFNumberRef bytes_per_element_number =
		CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &bytes_per_element);
	...
	CVReturn result =
		CVPixelBufferCreateWithIOSurface(kCFAllocatorDefault, surface, pixel_buffer_attributes, &pixel_buffer);
...
static void transfer_texture_to_iosurface(gs_texture_t *texture, IOSurfaceRef surface)
{
	...
	if (gs_stagesurface_map(stage_surface, &stage_data, &stage_linesize)) {
		...
		for (uint32_t y = 0; y < height; y++) {
			memcpy((uint8_t *)base_address + y * bytes_per_row, stage_data + y * stage_linesize, width * 4);
		}
...
static void process_segmentation_mask_async(struct optimized_vision_data *filter, CVPixelBufferRef input_buffer)
{
			...
			initWithCVPixelBuffer:input_buffer
				      options:@{
					      VNImageOptionCIContext:
						      [CIContext contextWithMTLDevice:filter->metal_device]
				      }];
...
static void optimized_vision_render(void *data, gs_effect_t *effect)
{
	...
	if (!filter->input_pixel_buffer || filter->cached_width != width || filter->cached_height != height) {
	...
	if (atomic_load(&filter->has_valid_mask) && filter->output_pixel_buffer) {
		...
		if (!filter->cached_mask_texture || filter->cached_width != width || filter->cached_height != height) {
			...
			filter->cached_mask_texture = gs_texture_create(
				...
				(uint32_t)CVPixelBufferGetHeight(filter->output_pixel_buffer), GS_A8, 1, &mask_data, 0);
		...
		if (obs_source_process_filter_begin(filter->context, format, OBS_ALLOW_DIRECT_RENDERING)) {
			...
			gs_blend_state_push();
			obs_source_process_filter_tech_end(filter->context, filter->composite_effect, 0, 0,
							   technique_name);
			gs_blend_state_pop();
		}
...
static obs_properties_t *optimized_vision_properties(void *unused)
{
	...
	obs_properties_add_group(props, "basic_settings", obs_module_text("BasicSettings"), OBS_GROUP_NORMAL,
				 basic_group);
	...
	obs_properties_add_group(props, "edge_settings", obs_module_text("EdgeRefinement"), OBS_GROUP_NORMAL,
				 edge_group);
	...
	obs_properties_add_float_slider(edge_group, "mask_brightness", obs_module_text("MaskBrightness"), -0.5, 0.5,
					0.05);

	// === SPILL SUPPRESSION GROUP ===
	obs_properties_t *spill_group = obs_properties_create();
	obs_properties_add_group(props, "spill_settings", obs_module_text("SpillSuppression"), OBS_GROUP_NORMAL,
				 spill_group);
	...
	obs_properties_add_group(props, "blend_settings", obs_module_text("AdvancedBlending"), OBS_GROUP_NORMAL,
				 blend_group);

	// Blend modes
	obs_property_t *blend_mode_list = obs_properties_add_list(
		blend_group, "blend_mode", obs_module_text("BlendMode"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	obs_properties_add_group(props, "arm_settings", obs_module_text("ARMOptimizations"), OBS_GROUP_NORMAL,
				 arm_group);
	...
	obs_properties_add_group(props, "preprocessing_settings", obs_module_text("Preprocessing"), OBS_GROUP_NORMAL,
				 preproc_group);

	// Input resolution scaling
	obs_property_t *input_scale_list = obs_properties_add_list(
		preproc_group, "input_scale", obs_module_text("InputScale"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	// Temporal smoothing
	obs_properties_add_float_slider(preproc_group, "temporal_smoothing", obs_module_text("TemporalSmoothing"), 0.0,
					1.0, 0.05);

	// === DEBUGGING GROUP ===
	obs_properties_t *debug_group = obs_properties_create();
	obs_properties_add_group(props, "debug_settings", obs_module_text("DebuggingTools"), OBS_GROUP_NORMAL,
				 debug_group);
	...
	// Debug logging level
	obs_property_t *debug_level_list = obs_properties_add_list(
		debug_group, "debug_level", obs_module_text("DebugLevel"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	obs_properties_add_group(props, "preset_settings", obs_module_text("Presets"), OBS_GROUP_NORMAL, preset_group);

	// Quality presets
	obs_property_t *preset_list = obs_properties_add_list(preset_group, "quality_preset",
							      obs_module_text("QualityPreset"), OBS_COMBO_TYPE_LIST,
...
static void optimized_vision_update(void *data, obs_data_t *settings)
{
	...
	if (filter->prefer_performance_cores) {
		...
		dispatch_queue_attr_t attr = dispatch_queue_attr_make_with_qos_class(
			...
			filter->thread_priority == 2 ? QOS_CLASS_USER_INTERACTIVE : QOS_CLASS_USER_INITIATED, 0);
	...
	try
	{
		...
		if (!filter->metal_device) {
			...
			if (!filter->metal_command_queue) {
				...
				CVReturn cv_result = CVMetalTextureCacheCreate(kCFAllocatorDefault, NULL,
									       filter->metal_device, NULL,
